...
export default function AppareilsPage() {
  ...
  return (
    <main className="min-h-[100svh] w-full bg-[var(--t2m-paper)] flex flex-col">
      <header className="shrink-0 flex items-center gap-2 px-3 border-b border-[var(--t2m-line)]" style={{ height: 'calc(env(safe-area-inset-top) + 3.25rem)', paddingTop: 'env(safe-area-inset-top)' }}>
        <button onClick={() => router.push('/profile')} aria-label="Retour" className="w-9 h-9 rounded-full grid place-items-center text-[var(--t2m-ink-2)]"><ChevronLeft className="w-6 h-6" /></button>
        <h1 className="text-[16px] font-semibold text-[var(--t2m-ink)]">Ordinateurs connectés</h1>
      </header>

      <div className="flex-1 overflow-y-auto p-4 max-w-lg w-full mx-auto">
        <p className="text-[12.5px] text-[var(--t2m-ink-3)] leading-relaxed mb-4">
        ...
        {loading ? (
          ...
          <div className="py-14 text-center px-6">
            <Monitor className="w-8 h-8 text-[var(--t2m-ink-3)] mx-auto mb-3" strokeWidth={1.6} />
            <p className="text-[var(--t2m-ink-3)] text-[14px]">Aucun ordinateur connecté.</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {list.map((s) => (
              <div key={s.pub_id} className="bg-white border border-[var(--t2m-line)] shadow-[0_2px_10px_rgba(47,52,58,.05)] rounded-2xl p-3.5 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-red-500/15 border border-red-400/25 grid place-items-center text-red-500 shrink-0"><Monitor className="w-5 h-5" /></div>
                <div className="flex-1 min-w-0">
                  <div className="text-[var(--t2m-ink)] text-[14px] font-semibold">
                  ...
                  <div className="text-[var(--t2m-ink-3)] text-[12px] mt-0.5">IP {s.ip || 'inconnue'} · connecté le {when(s.created_at)}</div>
                </div>
                <button onClick={() => revoke(s.pub_id)} disabled={busy === s.pub_id}
                  className="shrink-0 h-9 px-3 rounded-full bg-[var(--t2m-wash)] text-[var(--t2m-ink)] text-[12.5px] font-medium inline-flex items-center gap-1.5 disabled:opacity-50">
