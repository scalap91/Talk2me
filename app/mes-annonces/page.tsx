...
/**
 * « Mes annonces » — écran LISTE + bouton + (Pascal 2026-07-28, Phase 2 : cale le web sur le natif).
 * Miroir de MyAnnoncesScreen (talk2me-flutter) : d'ABORD la liste de mes annonces existantes, un
 * bouton + en bas dévoile le formulaire (DepositAnnonceSheet). On n'arrive JAMAIS direct au form.
 */
...
export default function MesAnnoncesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(Array.isArray(d?.annonces) ? d.annonces : []);
    } catch { setItems([]); }
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-10 flex items-center gap-2 px-3 h-14 bg-white border-b border-[#EDF0F4]">
        ...
        <h1 className="text-[17px] font-extrabold text-[#2F343A]" style={{ fontFamily: "'Outfit',sans-serif" }}>Mes annonces</h1>
      </header>
      ...
      <div className="px-4 pt-3 pb-28">
        {loading ? (
          ...
          <div className="text-center mt-16 px-6">
            ...
            <p className="text-[15px] font-bold text-[#2F343A]" style={{ fontFamily: "'Outfit',sans-serif" }}>Aucune annonce pour l’instant</p>
            <p className="text-[13px] text-[#6A7585] mt-1">Appuie sur + pour vendre ton premier objet.</p>
          </div>
        ) : (
              ...
              <li key={a.id}>
                <button type="button" onClick={() => setEdit(a)} className="w-full flex items-center gap-3 bg-[#F7F8FA] border border-[#EAECEF] rounded-2xl p-2.5 text-left active:scale-[0.99] transition">
                  <div className="w-14 h-14 shrink-0 rounded-xl bg-[#EDF0F4] overflow-hidden grid place-items-center">
                    {cover(a) ? <img src={cover(a) as string} alt="" className="w-full h-full object-cover" /> : <span className="text-[20px]">🏷️</span>}
                  </div>
      ...
      {/* Bouton + (identique natif : FAB en bas à droite qui dévoile le formulaire) */}
      <button
        type="button"
