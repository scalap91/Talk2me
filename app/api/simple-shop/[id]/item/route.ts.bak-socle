...
// Média SERVEUR : accepte /uploads/x OU l'URL absolue https://host/uploads/x (le natif envoie
// parfois l'absolue depuis le menu « attacher/créer ») → toujours stocké en RELATIF (portable dev↔beta).
function toRel(u: unknown): string | null {
...
export async function POST(req: NextRequest, ctx: Params) {
  ...
  if (!imageRel) return NextResponse.json({ error: 'image_required' }, { status: 400 });
...
export async function PATCH(req: NextRequest, ctx: Params) {
  ...
  if (body.action === 'edit') {
    item = updateItemFields(id, body.item_id, {
      ...
      photos: body.photos !== undefined ? (() => { const a = Array.isArray(body.photos) ? body.photos.map(toRel).filter((u): u is string => !!u).slice(0, 8) : []; return a.length ? JSON.stringify(a) : null; })() : undefined,
