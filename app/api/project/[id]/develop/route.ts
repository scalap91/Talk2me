...
import {
  PRODUCER_STEPS, nextProducerStep, buildProducerPrompt, applyProducerOutput, type ProducerStep,
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  let output = manual;
  if (output === null) {
    const { system, user: userPrompt } = buildProducerPrompt(project, step);
    output = await llmComplete(system, userPrompt, { temperature: 0.6, maxTokens: 1800, tag: 'film-producer' });
    if (output === null) {
