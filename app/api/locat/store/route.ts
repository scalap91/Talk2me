/** Talk2Me — LOCAT👀 : catalogue des BIENS À LOUER (recycle la boutique SHEIN, flag rental=1).
 *  GET → { categories:[{category, products:[{id,title,image,price_label,rate_unit}]}] }
 *  Public. Même forme que /api/shop/store → même storefront (SheinStore) réutilisé. */
import { NextResponse } from 'next/server';
import type { NextRequest } from 'next/server';
import { getRentalCatalog, getRentalNearby } from '@/lib/db';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

export function GET(req: NextRequest) {
  // PROXIMITÉ (Pascal 2026-09-06) : si l'utilisateur fournit sa position, on renvoie UNE liste
  // « 📍 Autour de moi » triée du plus proche au plus loin (LOCAT = location de proximité).
  // Sans géo → catalogue classique par catégories (repli).
  // TAXONOMIE 2 NIVEAUX : `family` (niveau 1) + `sub` (niveau 2) filtrent le catalogue.
  const family = (req.nextUrl.searchParams.get('family') || '').trim();
  const sub = (req.nextUrl.searchParams.get('sub') || '').trim();
  // ⚠️ Number(null)===0 et Number('')===0 : on exige que les params soient PRÉSENTS (non vides),
  // sinon un appel sans géo partait à tort en mode « autour de (0,0) ».
  const latS = req.nextUrl.searchParams.get('lat');
  const lngS = req.nextUrl.searchParams.get('lng');
  const lat = Number(latS);
  const lng = Number(lngS);
  if (latS && lngS && Number.isFinite(lat) && Number.isFinite(lng)) {
    const rParam = Number(req.nextUrl.searchParams.get('radius')); // rayon en km (5/10/50…), absent/0 = tout
    const radiusKm = Number.isFinite(rParam) && rParam > 0 ? rParam : null;
    const products = getRentalNearby({ lat, lng }, 80, radiusKm, { family: family || undefined, sub: sub || undefined });
    const label = [radiusKm ? `📍 Autour de moi · ${radiusKm} km` : '📍 Autour de moi', sub || family].filter(Boolean).join(' · ');
    return NextResponse.json({ ok: true, geo: true, radius: radiusKm, categories: products.length ? [{ category: label, products }] : [] });
  }
  // Sans géo : une FAMILLE → ses sous-catégories en onglets (niveau 2) ; sinon → les familles (niveau 1).
  const categories = getRentalCatalog(family ? 0 : 12, family ? { family } : undefined).filter((c) => c.products.length > 0);
  return NextResponse.json({ ok: true, geo: false, family: family || null, categories });
}
