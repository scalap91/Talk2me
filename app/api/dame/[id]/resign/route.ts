/**
 * POST /api/dame/[id]/resign
 * Talk2Me #408 (Pascal 2026-06-05) — Abandon dames.
 */
...
export async function POST(request: NextRequest, ctx: Params) {
  ...
  const updated = applyDameMove({
    gameId: id, state: game.state, moves: game.moves, status, winner: opponent,
  });
  if (!updated) return NextResponse.json({ error: 'apply_failed' }, { status: 500 });
  publish(`conv:${convId}`, {
    kind: 'game_end',
    data: {
      conversation_id: convId, game_id: id, game_kind: 'dame',
      winner: opponent, status, reason: 'resign', from_user_id: me.id,
    },
  });
