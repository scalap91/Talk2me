  ...
  return (
    <div className="min-h-[100svh] bg-[var(--t2m-paper)] text-[var(--t2m-ink)]">
      ...
      <div className="max-w-xl mx-auto p-4 space-y-4">
        {showNew && (
          <div className="rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] p-3 space-y-2">
            <p className="text-[13px] font-semibold text-[var(--t2m-ink-2)]">Nouveau bail (vous êtes le bailleur)</p>
            ...
            <p className="text-[10.5px] text-[var(--t2m-ink-3)]">Jour = date d'échéance chaque mois (1–28). Une échéance est générée chaque mois ; le locataire est relancé et paie en 1 clic.</p>
        ...
        {loading ? (
          <div className="grid place-items-center py-16 text-[var(--t2m-ink-3)]"><Loader2 className="w-6 h-6 animate-spin" /></div>
        ) : leases.length === 0 ? (
          <p className="text-center text-[var(--t2m-ink-3)] text-[13.5px] py-16">Aucun bail. En tant que bailleur, crée-en un avec « + Bail ».</p>
        ) : leases.map((l) => (
          <div key={l.id} className="rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] overflow-hidden">
            <div className="p-3 border-b border-[var(--t2m-line)] flex items-center justify-between">
              <div>
                <p className="text-[14px] font-semibold">{l.title || 'Bail'}</p>
                <p className="text-[11.5px] text-[var(--t2m-ink-3)]">{l.role === 'landlord' ? `Locataire : ${l.other_name || '—'}` : `Bailleur : ${l.other_name || '—'}`} · {formatMoney(l.monthly_cents)}/mois · le {l.day_of_month}</p>
              </div>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--t2m-wash)] text-[var(--t2m-ink-3)]">{l.role === 'landlord' ? 'Bailleur' : 'Locataire'}</span>
            </div>
            <div className="divide-y divide-[var(--t2m-line)]">
              {l.dues.map((d) => (
                <div key={d.id} className="flex items-center justify-between px-3 py-2.5">
                  <div className="flex items-center gap-2 text-[13px]">
                    {d.status === 'paid' ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <Clock className={`w-4 h-4 ${d.due_date < Date.now() ? 'text-amber-400' : 'text-[var(--t2m-ink-3)]'}`} />}
                    <span className="text-[var(--t2m-ink-2)]">{d.period}</span>
                    <span className="text-[var(--t2m-ink-3)]">· {formatMoney(d.amount_cents)}</span>
                  </div>
                  {d.status === 'paid' ? (
                    ...
                    <span className={`text-[11px] ${d.due_date < Date.now() ? 'text-amber-500' : 'text-[var(--t2m-ink-3)]'}`}>{d.due_date < Date.now() ? 'En retard' : 'En attente'}</span>
                  )}
                </div>
              ))}
            </div>
