 ...
 * le VALIDATEUR tranche (remboursement + sanction). Réservé chef (rang≥3)/validateur. Le remboursement
 * réel n'est PAS exécuté ici (money gaté) — la décision est enregistrée, signée.
 */
...
interface L { id: string; subject_name: string; opener_name: string; chef_name: string; reason: string; chef_report: string | null }
...
export default function LitigesPage() {
  ...
  const decide = async (id: string) => {
    ...
    setBusy(id);
    try { const d = await fetch('/api/litige', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'decide', litige_id: id, refund_type: dd.refund, sanction_level: dd.level || null, note: dd.note }) }).then((r) => r.json()); if (d?.ok) await load(); else alert(d?.message || 'Échec.'); } finally { setBusy(''); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FBFAF8] text-[#1A1D22] px-5 py-6 pb-24">
      <div className="max-w-[640px] mx-auto">
        ...
        <p className="text-[14px] text-[#6E7480] mb-5">Le chef <b>instruit</b> (rapport signé), le validateur <b>tranche</b> (remboursement + sanction). On juge sur les faits. Le remboursement réel est <b>enregistré, pas encore exécuté</b> (money gaté).</p>
            ...
            {data.instructed.map((l) => {
              ...
              return (
                <div key={l.id} className={card}>
                  ...
                  <button onClick={() => decide(l.id)} disabled={busy === l.id} className="mt-2 w-full py-2.5 rounded-lg bg-[#E24C4C] text-white font-semibold text-[13.5px] disabled:opacity-50">{busy === l.id ? '…' : 'Trancher (signé) — remboursement gaté'}</button>
                </div>
              );
            })}
          </>
