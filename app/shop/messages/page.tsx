  ...
  return (
    <div className="fixed inset-0 z-[60] bg-[#0e0e12] text-white flex flex-col">
      ...
      <header className="shrink-0 flex items-center gap-2 px-3 h-12 border-b border-white/8">
        <button onClick={() => router.push('/shop/vous')} aria-label="Retour" className="w-9 h-9 rounded-full grid place-items-center text-white/80"><ChevronLeft className="w-6 h-6" /></button>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center px-6">
              <MessageCircle className="w-8 h-8 text-white/25 mx-auto mb-3" strokeWidth={1.6} />
              <p className="text-white/60 text-[14px]">Aucun message vendeur pour l’instant.</p>
            </div>
          ) : (
            <ul className="divide-y divide-white/5">
              {list.map((c) => (
                <li key={c.id}>
                  <button onClick={() => router.push(`/c/${c.id}`)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/[0.04] text-left">
                    <span className="w-11 h-11 rounded-full overflow-hidden bg-white/10 grid place-items-center text-white/60 font-bold shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[14.5px] font-medium text-white/95 truncate">{c.peer?.display_name || c.peer?.username || 'Vendeur'}</div>
                      <div className="text-[12.5px] text-white/55 truncate">{c.last_message_preview || 'Nouvelle conversation'}</div>
                    </div>
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      <span className="text-[11px] text-white/40">{when(c.last_message_at)}</span>
