...
export default function ShopVousPage() {
  ...
  const Row = ({ icon: Icon, label, sub, onClick }: { icon: typeof Package; label: string; sub?: string; onClick: () => void }) => (
    <button onClick={onClick} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-[var(--t2m-wash)] active:bg-[var(--t2m-wash)] text-left">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[14.5px] text-[var(--t2m-ink)] font-medium">{label}</div>
        {sub && <div className="text-[12px] text-[var(--t2m-ink-3)]">{sub}</div>}
      </div>
      <ChevronRight className="w-5 h-5 text-[var(--t2m-ink-3)]" />
    </button>
  );
      ...
      <div className="flex-1 min-h-0 overflow-y-auto pb-24 md:pb-6">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-3 p-4">
            <div className="w-14 h-14 rounded-full overflow-hidden bg-[var(--t2m-wash)] grid place-items-center text-[var(--t2m-ink-3)] text-[20px] font-bold">
              {me?.avatar_url
            ...
            <div className="min-w-0">
              ...
              {me?.username && <div className="text-[12.5px] text-[var(--t2m-ink-3)]">@{me.username}</div>}
            </div>
          </div>

          <div className="mt-1 divide-y divide-[var(--t2m-line)] border-y border-[var(--t2m-line)]">
