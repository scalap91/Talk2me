...
const DOT: Record<BlockStatus, string> = { draft: 'bg-neutral-400', rendered: 'bg-emerald-500', modified: 'bg-amber-500', error: 'bg-red-500' };
...
export default function ComposerProjectEditor({ initialPrompt, initialProjectId }: { initialPrompt?: string; initialProjectId?: string }) {
  ...
  if (!project) {
    return (
      <div className="mx-auto max-w-xl px-4 pt-10 text-[var(--t2m-ink)]">
        <div className="rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] p-5">
          <label className="mb-2 flex items-center gap-2 text-sm text-[var(--t2m-ink-2)]"><Sparkles className="h-4 w-4 text-[var(--t2m-primary)]" /> Décris ce que tu veux créer</label>
          <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="ex. fais une vidéo sur cet hôtel…"
            className="h-28 w-full resize-none rounded-xl border border-[var(--t2m-line)] bg-[var(--t2m-wash)] p-3 text-[15px] outline-none placeholder:text-[var(--t2m-ink-3)] focus:border-[var(--t2m-primary)]" />
          <button onClick={create} disabled={creating || !prompt.trim()}
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--t2m-primary)] py-3 font-medium text-white disabled:opacity-40">
  ...
  // ====================== STUDIO ======================
  return (
    <div className="mx-auto max-w-3xl px-3 pb-28 pt-4 text-[var(--t2m-ink)]">
      {/* haut : PLAN IA (gauche) + APERÇU (centre/droite) */}
      <div className="grid gap-3 lg:grid-cols-[minmax(0,360px)_1fr]">

        {/* ---------- PLAN IA ---------- */}
        <div className="space-y-3">
          <div className="rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] p-4">
            <div className="mb-1 text-[11px] uppercase tracking-wide text-[var(--t2m-ink-3)]">Plan IA</div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5 text-[11px] text-[var(--t2m-ink-2)]">
              <span className="rounded-full bg-[var(--t2m-wash)] px-2 py-0.5 capitalize">{project.intent}</span>
              <span className="rounded-full bg-[var(--t2m-wash)] px-2 py-0.5">{project.format.replace('_', ' ')}</span>
              <span className="rounded-full bg-[var(--t2m-wash)] px-2 py-0.5">{project.ratio}</span>
              <span className="inline-flex items-center gap-1 rounded-full bg-[var(--t2m-wash)] px-2 py-0.5"><Dot s={project.status} /> {LABEL[project.status]}</span>
            </div>
          </div>

          {/* contrôles globaux : voix / avatar / musique / sous-titres */}
          {isMedia && (
            <div className="rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] p-3">
              <div className="mb-2 text-[11px] uppercase tracking-wide text-[var(--t2m-ink-3)]">Pistes</div>
              <div className="grid grid-cols-2 gap-2 text-[12px]">
                <button onClick={() => patchGlobal({ voiceover: !project.voiceover })} className="flex items-center gap-2 rounded-lg border border-[var(--t2m-line)] px-2.5 py-2">
                  {project.voiceover ? <Mic className="h-3.5 w-3.5 text-emerald-500" /> : <MicOff className="h-3.5 w-3.5 text-[var(--t2m-ink-3)]" />} Voix {project.voiceover ? 'on' : 'off'}
                </button>
                <button onClick={() => patchGlobal({ presenter: !project.presenter })} className="flex items-center gap-2 rounded-lg border border-[var(--t2m-line)] px-2.5 py-2">
                  <User className={`h-3.5 w-3.5 ${project.presenter ? 'text-[var(--t2m-primary)]' : 'text-[var(--t2m-ink-3)]'}`} /> Avatar Léa {project.presenter ? 'on' : 'off'}
                </button>
                <div className="flex items-center gap-2 rounded-lg border border-[var(--t2m-line)] px-2.5 py-2 text-[var(--t2m-ink-2)]"><Music className="h-3.5 w-3.5" /> Musique <span className="ml-auto"><Dot s={project.music.status} /></span></div>
                <div className="flex items-center gap-2 rounded-lg border border-[var(--t2m-line)] px-2.5 py-2 text-[var(--t2m-ink-2)]"><Captions className="h-3.5 w-3.5" /> Sous-titres</div>
              </div>
            </div>
          )}
          ...
          {isText && (
            <div className="rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] p-4">
              <div className="mb-2 text-[11px] uppercase tracking-wide text-[var(--t2m-ink-3)]">Texte</div>
              <textarea defaultValue={project.text} onBlur={(e) => patchGlobal({ text: e.target.value })}
                className="h-56 w-full resize-none rounded-xl border border-[var(--t2m-line)] bg-[var(--t2m-wash)] p-3 text-[15px] outline-none focus:border-[var(--t2m-primary)]" />
            </div>
          )}
        </div>
      ...
      {/* ---------- TIMELINE ---------- */}
      {isMedia && (
        <div className="mt-3 rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] p-3">
          <div className="mb-2 text-[11px] uppercase tracking-wide text-[var(--t2m-ink-3)]">Timeline</div>
            ...
            {project.scenes.map((s, i) => {
              ...
              return (
                <button key={s.id} onClick={() => setSel(s.id)}
                  className={`relative w-28 shrink-0 overflow-hidden rounded-xl border text-left ${active ? 'border-[var(--t2m-primary)]' : 'border-[var(--t2m-line)]'}`}>
      ...
      {isMedia && scene && (
        <div className="mt-3 rounded-2xl border border-[var(--t2m-line)] bg-white shadow-[0_2px_10px_rgba(47,52,58,.05)] p-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[11px] uppercase tracking-wide text-[var(--t2m-ink-3)]">Scène {project.scenes.findIndex((s) => s.id === scene.id) + 1}</span>
            <div className="flex gap-1.5">
              {[['image', 'Image'], ['voice', 'Voix'], project.presenter ? ['avatar', 'Léa'] : null].filter(Boolean).map((b: any) => (
                <button key={b[0]} onClick={() => render({ sceneId: scene.id, blocks: [b[0]] })} disabled={!!regen}
                  className="rounded-full border border-[var(--t2m-line)] px-2 py-0.5 text-[11px] text-[var(--t2m-ink-2)] disabled:opacity-40">↻ {b[1]}</button>
              ))}
              <button onClick={() => render({ sceneId: scene.id })} disabled={!!regen}
                className="inline-flex items-center gap-1 rounded-full bg-[var(--t2m-primary)] px-2.5 py-0.5 text-[11px] font-medium text-white disabled:opacity-40">
          ...
          <label className="mb-1 block text-[11px] text-[var(--t2m-ink-3)]">Script (→ voix + lèvres + sous-titres)</label>
          <textarea value={scene.script} onChange={(e) => patchScene(scene.id, { script: e.target.value })}
            className="h-20 w-full resize-none rounded-lg border border-[var(--t2m-line)] bg-[var(--t2m-wash)] p-2.5 text-[14px] outline-none focus:border-[var(--t2m-primary)]" />
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-[11px] text-[var(--t2m-ink-3)]">Sous-titre incrusté (n'altère pas la vidéo)</label>
              <input value={scene.caption} onChange={(e) => patchScene(scene.id, { caption: e.target.value })}
                className="w-full rounded-lg border border-[var(--t2m-line)] bg-[var(--t2m-wash)] px-2.5 py-2 text-[13px] outline-none focus:border-[var(--t2m-primary)]" />
            </div>
            <div>
              <label className="mb-1 block text-[11px] text-[var(--t2m-ink-3)]">Visuel (→ régénère seulement cette image)</label>
              <input value={scene.visual_prompt} onChange={(e) => patchScene(scene.id, { visual_prompt: e.target.value })}
                className="w-full rounded-lg border border-[var(--t2m-line)] bg-[var(--t2m-wash)] px-2.5 py-2 text-[13px] text-[var(--t2m-ink-2)] outline-none focus:border-[var(--t2m-primary)]" />
            </div>
          </div>
          ...
          {(motionScene === scene.id || scene.motion?.url || (scene.motion?.error && scene.motion.error !== null)) && (
            <div className="mt-3 rounded-xl border border-cyan-200 bg-cyan-50 p-3">
              <div className="mb-2 flex items-center gap-2 text-[11px] uppercase tracking-wide text-cyan-600">
                Scène vivante {scene.motion?.url && motionScene !== scene.id && '· dans le montage'}
              </div>
              {motionScene === scene.id ? (
                <div className="py-2">
                  <div className="mb-2 flex items-center gap-2 text-sm text-[var(--t2m-ink-2)]">
                    <Loader2 className="h-4 w-4 animate-spin text-cyan-500" />
                    Animation sur ton GPU… {motionElapsed}s <span className="text-[var(--t2m-ink-3)]">/ ~2-3 min — tu peux continuer à éditer</span>
                  </div>
                  {/* barre de progression (estimée sur ~150 s) */}
                  <div className="h-2 w-full overflow-hidden rounded-full bg-[var(--t2m-wash)]">
                ...
                <div className="py-3 text-sm text-amber-600">Animation indisponible ({scene.motion?.error}). Le montage garde l'image fixe.</div>
              )}
            </div>
          )}
        </div>
      ...
      {publishedId && <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">Publié dans le feed ✓</div>}

      {/* ---------- BARRE D'ACTIONS ---------- */}
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--t2m-line)] bg-[var(--t2m-paper)]/95 px-3 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center gap-2">
          <button onClick={() => { setProject(null); setSel(null); setPublishedId(null); }} className="inline-flex items-center gap-1.5 rounded-xl border border-[var(--t2m-line)] px-3 py-2.5 text-sm text-[var(--t2m-ink-2)]"><Plus className="h-4 w-4" /> Nouveau</button>
          {isMedia && (
            <button onClick={() => render()} disabled={rendering || !!regen}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-[var(--t2m-line)] bg-[var(--t2m-wash)] py-3 font-medium disabled:opacity-40">
          ...
          <button onClick={useInComposer} disabled={publishing}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--t2m-primary)] py-3 font-medium text-white disabled:opacity-40">
