...
export default function SanctionPanel({ userId, name, onClose }: { userId: string; name: string; onClose: () => void }) {
  ...
  const openLitige = async () => {
    const reason = window.prompt(`Ouvrir un différend avec ${name} — motif (les faits) :`);
    if (!reason || !reason.trim() || busy) return;
    setBusy(true);
    try { const d = await fetch('/api/litige', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'open', subject_id: userId, reason: reason.trim() }) }).then((r) => r.json()); alert(d?.ok ? 'Différend ouvert. À examiner par un chef, puis décidé par un validateur (page 🤝 Médiation).' : 'Échec.'); } catch { /* */ } finally { setBusy(false); }
  };
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 140, background: 'rgba(20,20,26,.55)', display: 'grid', placeItems: 'end center' }} onClick={onClose}>
      <div style={{ background: '#FBFAF8', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, maxHeight: '92dvh', overflowY: 'auto', padding: 18, paddingBottom: 'calc(env(safe-area-inset-bottom) + 20px)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div><div style={{ fontWeight: 800, fontSize: 17 }}>🤝 Gouvernance — {name}</div><div style={{ fontSize: 12, color: '#9AA0A8' }}>On s'appuie sur les faits, avec motif. Jamais à la tête du client.</div></div>
        ...
        {loading ? <div style={{ display: 'grid', placeItems: 'center', padding: 40 }}><Loader2 className="w-5 h-5 animate-spin" style={{ color: '#9AA0A8' }} /></div> : casier && (
          <>
            ...
            <div style={{ background: '#fff', border: '1px solid #ECEAE6', borderRadius: 14, padding: 14, marginBottom: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>{cell('Churn', casier.churn)}{cell('Plaintes', casier.reports)}{cell('Rembours.', casier.refunds)}{cell('Différends', casier.litiges)}</div>
              {casier.suggested > 0 && <div style={{ fontSize: 12, color: '#E8890C', marginTop: 10 }}>La data appelle un <b>niveau {casier.suggested}</b>.</div>}
              <button onClick={openLitige} disabled={busy} style={{ marginTop: 10, width: '100%', padding: '9px', borderRadius: 10, border: '1px solid #E3E6EA', background: '#fff', color: '#6E7480', fontSize: 12.5, fontWeight: 600 }}>⚠ Ouvrir un différend (→ chef examine, validateur décide)</button>
            </div>

            {/* POSER UNE SANCTION */}
            <div style={{ background: '#fff', border: '1px solid #ECEAE6', borderRadius: 14, padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, marginBottom: 8 }}>Poser une mesure</div>
              ...
              <button onClick={apply} disabled={!level || busy} style={{ marginTop: 10, width: '100%', padding: 12, borderRadius: 10, border: 'none', background: !level ? '#EDEBE7' : '#E24C4C', color: !level ? '#9AA0A8' : '#fff', fontWeight: 700, fontSize: 14 }}>{busy ? '…' : 'Poser la mesure (signée à mon nom)'}</button>
            </div>
            ...
            {history.length === 0 ? <div style={{ fontSize: 13, color: '#9AA0A8', padding: '0 2px 8px' }}>Aucune mesure. Casier propre.</div> : history.map((h) => (
              <div key={h.id} style={{ background: '#fff', border: '1px solid #ECEAE6', borderRadius: 12, padding: '10px 12px', marginBottom: 8, opacity: h.active ? 1 : 0.55 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {h.active === 1 && <button onClick={() => lift(h.id)} disabled={busy} style={{ fontSize: 12, color: '#12B76A', background: 'none', border: 'none', fontWeight: 600 }}>Retirer</button>}
                </div>
