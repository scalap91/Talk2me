...
export default function ContributorSimulator({ pages = false }: { pages?: boolean }) {
  ...
  const Sec = ({ children }: { children: React.ReactNode }) => pages
    ? <div className="shrink-0 basis-full snap-center snap-always overflow-y-auto" style={{ height: '100%', background: PANE, color: INK, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}><div style={{ padding: 'calc(env(safe-area-inset-top) + 72px) 16px calc(env(safe-area-inset-bottom) + 128px)', maxWidth: 560, width: '100%', marginLeft: 'auto', marginRight: 'auto' }}>{children}</div></div>
