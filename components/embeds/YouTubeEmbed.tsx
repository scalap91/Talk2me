 ...
 *
 * La vidéo démarre TOUJOURS au début, uniquement quand l'utilisateur clique
 * sur play (pas d'autoplay, pas de réglage d'instant de départ — Pascal 2026-06-21).
 */
export default function YouTubeEmbed({ videoId, originalUrl, rich }: YouTubeEmbedProps) {
  ...
  useEffect(() => {
    ...
    const onLoad = () => {
      try {
        win.postMessage(JSON.stringify({ event: 'listening', id: participantId }), '*');
      } catch {
        // ignore
      }
    };
    ...
    const onMessage = (e: MessageEvent) => {
      ...
      if (state === 1) {
        // PLAYING → demande le canal audio (coupe les autres vidéos)
  ...
  // URL avec jsapi activé + origin pour sécurité postMessage. PAS d'autoplay,
  // PAS de paramètre start → la vidéo démarre au début, au clic de l'utilisateur.
  const embedSrc =
      ...
      <div className="rounded-2xl overflow-hidden w-full max-w-full mx-auto bg-white/[0.04]">
        <div className="relative w-full pb-[56.25%] bg-black">
          <iframe
            ...
            allowFullScreen
            loading="lazy"
            allow="accelerated-2d-canvas; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
        ...
        <div className="p-3.5 space-y-2">
          ...
          {rich.channel && <div className="text-[12px] text-white/55">{rich.channel}</div>}
