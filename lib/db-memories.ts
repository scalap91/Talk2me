...
/**
 * lib/db-memories — section « memories » (mémoires long terme de l'IA perso,
 * isolées strictement par user_id) extraite du monolithe lib/db.ts (façade #53,
 * Pascal 2026-06-30). lib/db.ts re-exporte → appelants inchangés. Connexion =
 * getDb(). Table ai_memories toujours créée par l'init de lib/db.ts.
...
function parseAiMemoryRow(row: any): DbAiMemory {
  const k = row.kind;
  const kind: AiMemoryKind = k === 'habit' || k === 'fact' || k === 'style' ? k : 'preference';
  return {
    id: row.id,
    user_id: row.user_id,
    kind,
...
export function addAiMemory(input: AddAiMemoryInput): DbAiMemory {
  ...
  const kind: AiMemoryKind = input.kind === 'habit' || input.kind === 'fact' || input.kind === 'style' ? input.kind : 'preference';
  const db = getDb();
  const id = randomUUID();
  const now = Date.now();
  ...
  ).run(id, userId, kind, content, weight, input.sourceConvId ?? null, input.sourceMessageId ?? null, now);
  return {
    id, user_id: userId, kind, content, weight,
...
export function getAiMemories(userId: string, limit: number = 20): DbAiMemory[] {
  ...
  const rows = db
    .prepare('SELECT * FROM ai_memories WHERE user_id = ? ORDER BY weight DESC, created_at DESC LIMIT ?')
    .all(userId, n) as any[];
...
export function deleteAiMemory(userId: string, memoryId: string): boolean {
  ...
  const db = getDb();
  const r = db.prepare('DELETE FROM ai_memories WHERE id = ? AND user_id = ?').run(memoryId, userId);
  return r.changes > 0;
}
...
export function touchAiMemoryUsage(memoryId: string): void {
  if (!memoryId) return;
  const db = getDb();
  db.prepare('UPDATE ai_memories SET last_used_at = ? WHERE id = ?').run(Date.now(), memoryId);
}
